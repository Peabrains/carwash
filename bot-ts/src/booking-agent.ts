...
async function submitBooking(state: SafeBookingState, context: BookingContext, chatId: string): Promise<string> {
  if(!state.serviceName||!state.dateIso||!state.time24h||!state.customerName||!state.customerPhone)return "I still need the complete booking details before submitting.";
  ...
  const result = await reserveFirestoreAppointment(chatId, { step: "confirm", bookingRequestId: state.bookingRequestId, serviceId: service.id, serviceName: service.name, durationMinutes: service.duration_minutes, priceMyr: service.price_myr, dateIso: state.dateIso, time24h: state.time24h, customerName: state.customerName, customerPhone: state.customerPhone });

...
export async function updateBookingSettings(patch) { const db = ensureSupabase(); const current = await getBookingSettings(); const { data, error } = await db.from('booking_settings').update({ ...patch, ...scope(), updated_at: new Date().toISOString() }).eq('id', current.id).select().single(); errorOrThrow(error, 'updating booking settings'); return data; }

...
import { createBooking, loadCatalogue, loadSlots, manageBooking } from './lib/public-booking.js';
...
async function pageCustomerBook() {
  ...
  try {
    ...
    root.innerHTML = `<section class="provider-discovery"><div class="eyebrow">Find a provider</div><h3>Compare local car washes</h3><p class="lead">See locations, services, prices and durations before choosing where to book.</p><input id="catalogueSearch" class="catalogue-search" type="search" placeholder="Search provider, location or service" aria-label="Search providers"><div id="providerCards" class="provider-cards">${discoveryCards}</div></section><section class="customer-manage card"><h3>Manage an existing booking</h3><p class="lead">Use your booking reference and phone number to view, cancel or move it.</p><form id="manageForm"><div class="field"><label for="manageReference">Booking reference</label><input id="manageReference" placeholder="WP-T1-20260823-ABC123" required></div><div class="field"><label for="managePhone">Phone number</label><input id="managePhone" type="tel" placeholder="012-3456789" required></div><button class="btn secondary" type="submit">Find my booking</button></form><div id="manageResult" role="status"></div></section>
    ...
    document.querySelectorAll('[data-choose-location]').forEach(button => button.onclick = () => { const [providerId, locationId] = button.dataset.chooseLocation.split('|'); providerEl.value = providerId; refreshLocations(); locationEl.value = locationId; refreshServices(); document.getElementById('customerForm').scrollIntoView({ behavior: 'smooth', block: 'start' }); });

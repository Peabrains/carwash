...
import { buildOnboardingPresentation, formatPlanPrice, ONBOARDING_STEPS } from './lib/provider-onboarding.js';
...
async function pageProviderVerification() {
  const myGen = ++renderGen;
  const staff = await requireStaff(myGen);
  if (!staff) return;
  if (staff.role !== 'owner') {
    app.innerHTML = shell('verification', '<div class="eyebrow">Marketplace verification</div><h2>Provider owner access only</h2><p class="lead">Only the provider owner can submit verification documents.</p>');
    wireNav(); return;
  }
  try {
    const [onboarding, verification] = await Promise.all([api.getProviderOnboarding(), api.getProviderVerification()]);
    if (myGen !== renderGen) return;
    const status = verification?.status || onboarding?.profile?.marketplace_status || 'not_submitted';
    const locked = status === 'pending_review';
    const reason = verification?.decision_reason ? `<div class="verification-reason"><strong>Review note</strong><p>${h(verification.decision_reason)}</p></div>` : '';
    app.innerHTML = shell('verification', `<header class="overview-head"><div><div class="eyebrow">Marketplace verification</div><h2>Verify your business</h2><p class="lead">This review controls public marketplace visibility only. You can keep using your dashboard, private booking link and manual bookings while waiting.</p></div><span class="verification-status status-${h(status)}">${h(status.replaceAll('_', ' '))}</span></header><section class="overview-panel verification-panel"><div class="verification-requirements"><h3>What we need</h3><ul><li>Your current SSM registration document</li><li>A clear photo of your operating storefront</li><li>PDF, JPEG or PNG; maximum 10 MB each</li></ul></div>${verification?.submitted_at ? `<p class="lead">Last submitted ${h(new Date(verification.submitted_at).toLocaleString('en-MY'))} · version ${h(verification.submission_version)}</p>` : ''}${reason}${locked ? '<div class="readiness-card ready"><strong>Under review</strong><p>You cannot replace this submission until Docket finishes the review.</p></div>' : `<form id="verificationForm" class="verification-form"><div class="field"><label for="verificationSsm">SSM document</label><input id="verificationSsm" type="file" accept="application/pdf,image/jpeg,image/png" required></div><div class="field"><label for="verificationStorefront">Storefront photo</label><input id="verificationStorefront" type="file" accept="application/pdf,image/jpeg,image/png" required></div><button class="btn" type="submit">${verification ? 'Submit updated documents' : 'Submit for review'}</button><p id="verificationMessage" class="lead" role="status"></p></form>`}</section>`);
    wireNav();
    document.getElementById('verificationForm')?.addEventListener('submit', async event => {
      event.preventDefault(); const form = event.currentTarget; const button = form.querySelector('button'); const message = document.getElementById('verificationMessage'); button.disabled = true;
      try {
        message.textContent = 'Uploading your SSM document…';
        const ssm = await api.uploadProviderVerificationFile('ssm', document.getElementById('verificationSsm').files[0]);
        message.textContent = 'Uploading your storefront photo…';
        const storefront = await api.uploadProviderVerificationFile('storefront', document.getElementById('verificationStorefront').files[0]);
        message.textContent = 'Submitting for review…';
        await api.submitProviderVerification({ ssmDocumentPath: ssm.path, storefrontPhotoPath: storefront.path });
        await pageProviderVerification();
      } catch (error) { button.disabled = false; message.textContent = error?.message || 'Could not submit verification.'; }
    });
  } catch (error) {
    if (myGen !== renderGen) return;
    app.innerHTML = shell('verification', `<div class="eyebrow">Marketplace verification</div><h2>Could not load verification</h2><p class="lead" style="color:#b3261e">${h(error?.message || 'Unknown error')}</p><button class="btn" id="retryVerification">Try again</button>`);
    wireNav(); document.getElementById('retryVerification').onclick = pageProviderVerification;
  }
}
...
async function pagePlatformAdmin() {
  ...
  try {
    ...
    const profiles = Object.fromEntries(data.profiles.map(item => [item.provider_id, item]));
    ...
    const reviewCards = data.verifications.map(verification => { const provider = data.providers.find(item => item.id === verification.provider_id) || {}; const profile = profiles[verification.provider_id] || {}; const snapshotVersion = Number(verification.submitted_snapshot?.profile_version || 0); return `<article class="verification-review-card" data-verification-card="${h(verification.id)}"><div class="platform-provider-head"><div><strong>${h(provider.name || verification.provider_id)}</strong><span>${h(profile.legal_name || '')} · ${h(profile.ssm_number || '')}</span></div><span class="verification-status status-${h(verification.status)}">${h(verification.status.replaceAll('_', ' '))}</span></div><dl class="platform-provider-meta"><div><dt>Submitted</dt><dd>${h(new Date(verification.submitted_at).toLocaleString('en-MY'))}</dd></div><div><dt>Version</dt><dd>${h(snapshotVersion)}</dd></div><div><dt>Operational setup</dt><dd>${h(onboarding[verification.provider_id]?.status || 'unknown')}</dd></div><div><dt>Operations</dt><dd>${profile.operations_suspended ? 'Suspended' : 'Active'}</dd></div></dl>${verification.decision_reason ? `<div class="verification-reason"><strong>Previous decision</strong><p>${h(verification.decision_reason)}</p></div>` : ''}<div class="verification-review-actions"><button class="btn secondary compact-btn" type="button" data-open-verification="${h(verification.id)}">Open documents</button>${verification.status === 'pending_review' ? `<textarea data-verification-reason="${h(verification.id)}" placeholder="Reason required for changes, rejection or suspension"></textarea><label class="check"><input type="checkbox" data-suspend-operations="${h(verification.id)}"> Also suspend private and manual bookings</label><div class="confirmation-actions"><button class="btn compact-btn" data-verification-decision="approved" data-verification-id="${h(verification.id)}" data-version="${h(snapshotVersion)}" type="button">Approve</button><button class="btn secondary compact-btn" data-verification-decision="changes_requested" data-verification-id="${h(verification.id)}" data-version="${h(snapshotVersion)}" type="button">Request changes</button><button class="btn secondary compact-btn" data-verification-decision="rejected" data-verification-id="${h(verification.id)}" data-version="${h(snapshotVersion)}" type="button">Reject</button><button class="btn secondary compact-btn" data-verification-decision="suspended" data-verification-id="${h(verification.id)}" data-version="${h(snapshotVersion)}" type="button">Suspend</button></div>` : ''}</div></article>`; }).join('') || '<p class="muted">No verification submissions yet.</p>';
    app.innerHTML = shell('platform-admin', `<header class="overview-head"><div><div class="eyebrow">Docket platform</div><h2>Providers & subscriptions</h2><p class="lead">Review marketplace applications and manage provider plans.</p></div><span class="overview-date">Platform admin</span></header><section class="overview-metrics"><article><span>Providers</span><strong>${data.providers.length}</strong><small>${activeProviders} active</small></article><article><span>Pending reviews</span><strong>${data.verifications.filter(item => item.status === 'pending_review').length}</strong><small>marketplace applications</small></article><article><span>Locations</span><strong>${data.locations.filter(item => item.is_active !== false).length}</strong><small>active outlets</small></article><article><span>Attention</span><strong class="${attention.length ? 'metric-alert' : ''}">${attention.length}</strong><small>past due, blocked or paused</small></article></section><section class="overview-panel platform-verification-list"><div class="section-heading"><div><h3>Marketplace review queue</h3><p>Approval publishes a provider. Suspension can hide only the listing or stop all booking operations.</p></div><span class="section-count">${data.verifications.length}</span></div>${reviewCards}</section><section class="overview-panel platform-provider-list"><div class="section-heading"><div><h3>Provider accounts</h3><p>Choose a plan, then test payment. No real money is moved.</p></div><span class="section-count">${data.providers.length}</span></div>${rows}</section><section class="overview-panel platform-plan-list"><div class="section-heading"><div><h3>Manage plans</h3><p>Edit pricing and account limits. These are catalogue settings, not payment status.</p></div><span class="section-count">${data.plans.length}</span></div>${planCards}<div class="platform-new-plan"><input id="newPlanId" placeholder="plan-id"><input id="newPlanName" placeholder="Plan name"><input id="newPlanPrice" type="number" min="0" step="0.01" placeholder="Monthly RM"><button class="btn compact-btn" type="button" id="addPlan">Add plan</button></div></section>`);
    document.querySelectorAll('[data-open-verification]').forEach(button => button.onclick = async () => { const verification = data.verifications.find(item => item.id === button.dataset.openVerification); button.disabled = true; button.textContent = 'Preparing secure links…'; try { const urls = await api.getProviderVerificationDocumentUrls(verification); const links = document.createElement('div'); links.className = 'verification-document-links'; links.innerHTML = `<a class="btn secondary compact-btn" target="_blank" rel="noopener" href="${h(urls.ssm)}">Open SSM document</a><a class="btn secondary compact-btn" target="_blank" rel="noopener" href="${h(urls.storefront)}">Open storefront photo</a><small>Links expire in 5 minutes.</small>`; button.replaceWith(links); } catch (error) { button.disabled = false; button.textContent = 'Open documents'; alert(error?.message || 'Could not open documents.'); } });
    document.querySelectorAll('[data-verification-decision]').forEach(button => button.onclick = async () => { const id = button.dataset.verificationId; const reason = document.querySelector(`[data-verification-reason="${id}"]`)?.value || ''; const suspendOperations = document.querySelector(`[data-suspend-operations="${id}"]`)?.checked || false; button.disabled = true; try { await api.decideProviderVerification({ verificationId: id, decision: button.dataset.verificationDecision, reason, suspendOperations, expectedVersion: Number(button.dataset.version) }); await pagePlatformAdmin(); } catch (error) { button.disabled = false; alert(error?.message || 'Could not save review decision.'); } });
...
async function pageStaffBilling() {
  ...
  try {
    ...
    if (myGen !== renderGen) return;
    const subscription = data.subscription || {}; const plan = data.current_plan || {};
    const status = subscription.status || 'not started';
    const renewal = subscription.current_period_end; const usage = data.usage || {};
    const planOptions = data.plans.map(item => `<option value="${h(item.id)}" ${item.id === subscription.plan_id ? 'selected' : ''}>${h(item.name)} · ${h(formatPlanPrice(item))}</option>`).join('');
    const usageCard = (label, used, limit) => `<article><span>${h(label)}</span><strong>${Number(used || 0)} / ${limit ?? '∞'}</strong><small>${limit == null ? 'No plan limit' : `${Math.max(0, Number(limit) - Number(used || 0))} remaining`}</small></article>`;
    app.innerHTML = shell('billing', `<header class="settings-page-head"><div><div class="eyebrow">Account</div><h2>Plan & usage</h2><p class="lead">All plans are free during the pilot. No card or payment is required.</p></div><span class="history-status">${h(status.replaceAll('_', ' '))}</span></header><section class="billing-summary-grid"><article><span>Current plan</span><strong>${h(plan.name || 'No plan')}</strong><small>${h(plan.id ? formatPlanPrice(plan) : 'Choose a plan')}</small></article><article><span>Current period</span><strong>${renewal ? h(fmtDateTime(renewal)) : '—'}</strong><small>Usage resets at the next period</small></article><article><span>Pending change</span><strong>${h(data.pending_plan?.name || 'None')}</strong><small>${data.pending_plan ? `Changes ${h(fmtDateTime(subscription.pending_effective_at))}` : 'No scheduled downgrade'}</small></article></section><section class="overview-metrics billing-usage">${usageCard('Active outlets',usage.locations,plan.max_locations)}${usageCard('Staff + invited',Number(usage.staff||0)+Number(usage.pending_invites||0),plan.max_staff)}${usageCard('Bookings this period',usage.bookings,plan.max_monthly_bookings)}</section><section class="overview-panel billing-panel"><div class="section-heading"><div><h3>Change pilot plan</h3><p>Upgrades apply now. Downgrades apply at the next period after Docket checks your current usage fits.</p></div></div><div class="billing-plan-actions"><select id="billingPlan" aria-label="Choose a plan">${planOptions || '<option>No plans available</option>'}</select><button class="btn compact" id="changeBillingPlan" type="button">Change plan</button>${data.pending_plan ? '<button class="btn secondary compact" id="cancelPendingPlan" type="button">Cancel pending change</button>' : ''}</div><p id="billingMessage" class="lead" role="status"></p></section>`);
    document.getElementById('changeBillingPlan')?.addEventListener('click', async event => { const planId = document.getElementById('billingPlan').value; const target = data.plans.find(item => item.id === planId); if (!target || planId === subscription.plan_id) return; if (!window.confirm(`Change to ${target.name}? ${formatPlanPrice(target)}.`)) return; event.currentTarget.disabled = true; try { const result = await api.changeProviderPlan(planId); alert(result.effective === 'immediate' ? 'Plan changed immediately.' : `Downgrade scheduled for ${fmtDateTime(result.subscription.pending_effective_at)}.`); await pageStaffBilling(); } catch (error) { event.currentTarget.disabled = false; document.getElementById('billingMessage').textContent = error?.message || 'Could not change the plan.'; } });
    document.getElementById('cancelPendingPlan')?.addEventListener('click', async event => { event.currentTarget.disabled = true; try { await api.cancelPendingProviderPlanChange(); await pageStaffBilling(); } catch (error) { event.currentTarget.disabled = false; document.getElementById('billingMessage').textContent = error?.message || 'Could not cancel the pending change.'; } });
    wireNav();
  } catch (error) {
    if (myGen !== renderGen) return;
    ...
    <main class="screen landing-screen">
      <section class="landing-hero" aria-labelledby="landing-title">
        ...
        <div class="landing-actions" aria-label="Booking options">
          ...
          <a class="landing-action" href="#/account"><span><strong>Sign in or create an account</strong><small>Save vehicles, view history and rebook faster</small></span><span aria-hidden="true">→</span></a>
        </div>
      </section>
...
async function pageCustomerAccountEntry() {
  const existing = await customerAuth.getSupabaseUser().catch(() => null);
  if (existing) { location.hash = '#/account'; router(); return; }
  app.innerHTML = `<div class="app-shell customer-account-shell"><div class="topbar"><a class="brand" href="#/"><div class="drop"></div>Docket</a><a class="topbar-link" href="#/book">Book as guest</a></div><main class="screen customer-account-screen"><div class="eyebrow">Customer account</div><h1>Your washes, in one place.</h1><p class="lead">Sign in to see bookings made with this account and reuse your saved vehicles. You can always book without an account.</p><div class="account-auth-grid"><form id="customerSignIn" class="card account-card"><h3>Sign in</h3><div class="field"><label for="customerEmail">Email</label><input id="customerEmail" type="email" autocomplete="email" required></div><div class="field"><label for="customerPassword">Password</label><input id="customerPassword" type="password" autocomplete="current-password" minlength="8" required></div><button class="btn" type="submit">Sign in</button><button class="text-button" id="showCustomerReset" type="button">Forgot password?</button></form><form id="customerSignUp" class="card account-card"><h3>Create account</h3><div class="field"><label for="newCustomerEmail">Email</label><input id="newCustomerEmail" type="email" autocomplete="email" required></div><div class="field"><label for="newCustomerPassword">Password</label><input id="newCustomerPassword" type="password" autocomplete="new-password" minlength="8" required></div><button class="btn secondary" type="submit">Create account</button></form></div><form id="customerResetRequest" class="card account-card reset-request" hidden><h3>Reset password</h3><p class="lead">Enter your email and we’ll send a secure reset link.</p><div class="field"><label for="customerResetEmail">Email</label><input id="customerResetEmail" type="email" autocomplete="email" required></div><button class="btn" type="submit">Send reset link</button></form><p id="customerAuthMessage" class="lead" role="status"></p></main></div>`;
  const message = document.getElementById('customerAuthMessage');
  document.getElementById('showCustomerReset').onclick = () => { document.getElementById('customerResetRequest').hidden = false; };
  document.getElementById('customerSignIn').onsubmit = async event => { event.preventDefault(); message.textContent = 'Signing in…'; try { await customerAuth.signInCustomer(document.getElementById('customerEmail').value, document.getElementById('customerPassword').value); location.hash = '#/account'; router(); } catch (error) { message.textContent = error?.message || 'Could not sign in.'; } };
  document.getElementById('customerSignUp').onsubmit = async event => { event.preventDefault(); message.textContent = 'Creating your account…'; try { const result = await customerAuth.signUpCustomer(document.getElementById('newCustomerEmail').value, document.getElementById('newCustomerPassword').value); if (result?.session) { location.hash = '#/account'; router(); } else message.textContent = 'Check your email to confirm your account, then sign in.'; } catch (error) { message.textContent = error?.message || 'Could not create the account.'; } };
  document.getElementById('customerResetRequest').onsubmit = async event => { event.preventDefault(); try { await customerAuth.sendCustomerPasswordReset(document.getElementById('customerResetEmail').value); message.textContent = 'If that email is registered, a password reset link has been sent.'; } catch { message.textContent = 'If that email is registered, a password reset link has been sent.'; } };
}

async function pageCustomerPasswordReset() {
  app.innerHTML = `<div class="app-shell customer-account-shell"><div class="topbar"><a class="brand" href="#/"><div class="drop"></div>Docket</a></div><main class="screen customer-account-screen"><div class="eyebrow">Customer account</div><h1>Choose a new password.</h1><form id="customerPasswordReset" class="card account-card"><div class="field"><label for="customerNewPassword">New password</label><input id="customerNewPassword" type="password" autocomplete="new-password" minlength="8" required></div><div class="field"><label for="customerConfirmPassword">Confirm password</label><input id="customerConfirmPassword" type="password" autocomplete="new-password" minlength="8" required></div><button class="btn" type="submit">Update password</button><p id="customerResetMessage" class="lead" role="status"></p></form></main></div>`;
  try { await customerAuth.finishSupabaseRedirect(); } catch (error) { document.getElementById('customerResetMessage').textContent = error?.message || 'This reset link is invalid or expired.'; }
  document.getElementById('customerPasswordReset').onsubmit = async event => { event.preventDefault(); const message = document.getElementById('customerResetMessage'); const password = document.getElementById('customerNewPassword').value; if (password !== document.getElementById('customerConfirmPassword').value) { message.textContent = 'Passwords do not match.'; return; } try { await customerAuth.updateCustomerPassword(password); history.replaceState({}, document.title, `${location.pathname}#/account`); router(); } catch (error) { message.textContent = error?.message || 'Could not update your password.'; } };
}

async function pageCustomerAccount() {
  try { await customerAuth.finishSupabaseRedirect(); } catch {}
  const user = await customerAuth.getSupabaseUser().catch(() => null);
  if (!user) { location.hash = '#/account/login'; router(); return; }
  app.innerHTML = `<div class="app-shell customer-account-shell"><div class="topbar"><a class="brand" href="#/"><div class="drop"></div>Docket</a><button class="topbar-link text-button" id="customerSignOut" type="button">Sign out</button></div><main class="screen customer-account-screen"><div class="eyebrow">Customer account</div><h1>Welcome back.</h1><div id="customerAccountContent" class="card">Loading your bookings…</div></main></div>`;
  document.getElementById('customerSignOut').onclick = async () => { await customerAuth.signOutSupabase(); location.hash = '#/'; router(); };
  const root = document.getElementById('customerAccountContent');
  try {
    const [profileRow, vehicles, bookings, catalogue] = await Promise.all([customerAuth.getCustomerProfile(user), customerAuth.getCustomerVehicles(user), customerAuth.getCustomerBookings(user), loadCatalogue().catch(() => ({ providers: [], locations: [], services: [] }))]);
    const profile = normalizeCustomerProfile(profileRow, user); const grouped = groupCustomerBookings(bookings); const providers = Object.fromEntries((catalogue.providers || []).map(item => [item.id, item])); const locations = Object.fromEntries((catalogue.locations || []).map(item => [item.id, item])); const services = Object.fromEntries((catalogue.services || []).map(item => [item.id, item]));
    const bookingCard = booking => `<article class="account-booking"><div><strong>${h(services[booking.service_id]?.name || 'Car wash booking')}</strong><small>${h(providers[booking.provider_id]?.name || '')}${locations[booking.location_id]?.name ? ` · ${h(locations[booking.location_id].name)}` : ''}</small></div><div><span>${h(booking.scheduled_date || String(booking.scheduled_at).slice(0, 10))}</span><small>${h(new Date(booking.scheduled_at).toLocaleTimeString('en-MY', { hour: '2-digit', minute: '2-digit' }))} · ${h(booking.status)}</small></div><div><span>${h(booking.vehicle_plate || '')}</span><small>${h(booking.vehicle_make_model || '')}</small></div><strong>${h(booking.reference)}</strong></article>`;
    root.className = 'customer-account-content'; root.innerHTML = `<section class="card account-section"><div class="account-section-head"><div><h2>Upcoming bookings</h2><p class="lead">Bookings made while signed in appear here.</p></div><a class="btn compact" href="#/book">Book a wash</a></div><div class="account-bookings">${grouped.upcoming.length ? grouped.upcoming.map(bookingCard).join('') : '<p class="lead">No upcoming bookings yet.</p>'}</div></section><section class="account-grid"><form id="customerProfileForm" class="card account-section"><h2>Your details</h2><div class="field"><label>Email</label><input value="${h(profile.email)}" disabled></div><div class="field"><label for="accountName">Name</label><input id="accountName" value="${h(profile.name)}" autocomplete="name"></div><div class="field"><label for="accountPhone">Phone</label><input id="accountPhone" value="${h(profile.phone)}" autocomplete="tel"></div><button class="btn" type="submit">Save details</button></form><section class="card account-section"><h2>Saved vehicles</h2><div class="vehicle-list">${vehicles.map(vehicle => `<div class="saved-vehicle"><div><strong>${h(vehicle.plate)}</strong><small>${h(vehicle.make_model)}${vehicle.category ? ` · ${h(vehicle.category)}` : ''}</small></div>${vehicle.is_default ? '<span>Default</span>' : ''}<button class="text-button" data-delete-vehicle="${h(vehicle.id)}" type="button">Remove</button></div>`).join('') || '<p class="lead">No saved vehicles yet.</p>'}</div><form id="customerVehicleForm"><div class="field"><label for="vehiclePlate">Plate</label><input id="vehiclePlate" required></div><div class="field"><label for="vehicleMakeModel">Make and model</label><input id="vehicleMakeModel" required></div><div class="field"><label for="vehicleCategory">Category <span class="muted">(optional)</span></label><input id="vehicleCategory" placeholder="Sedan, SUV…"></div><label class="check"><input id="vehicleDefault" type="checkbox"> Use as default</label><button class="btn secondary" type="submit">Save vehicle</button></form></section></section><section class="card account-section"><h2>Booking history</h2><div class="account-bookings">${grouped.history.length ? grouped.history.map(bookingCard).join('') : '<p class="lead">No previous bookings yet.</p>'}</div></section><p id="accountMessage" class="lead" role="status"></p>`;
    const message = document.getElementById('accountMessage');
    document.getElementById('customerProfileForm').onsubmit = async event => { event.preventDefault(); try { await customerAuth.saveCustomerProfile(user, { name: document.getElementById('accountName').value, phone: document.getElementById('accountPhone').value }); message.textContent = 'Details saved.'; } catch (error) { message.textContent = error?.message || 'Could not save details.'; } };
    document.getElementById('customerVehicleForm').onsubmit = async event => { event.preventDefault(); try { await customerAuth.saveCustomerVehicle(user, { plate: document.getElementById('vehiclePlate').value, makeModel: document.getElementById('vehicleMakeModel').value, category: document.getElementById('vehicleCategory').value, isDefault: document.getElementById('vehicleDefault').checked }); pageCustomerAccount(); } catch (error) { message.textContent = error?.message || 'Could not save vehicle.'; } };
    document.querySelectorAll('[data-delete-vehicle]').forEach(button => button.onclick = async () => { try { await customerAuth.deleteCustomerVehicle(user, button.dataset.deleteVehicle); pageCustomerAccount(); } catch (error) { message.textContent = error?.message || 'Could not remove vehicle.'; } });
  } catch (error) { root.className = 'card'; root.innerHTML = `<h2>We couldn’t load your account</h2><p class="lead">${h(error?.message || 'Please try again.')}</p><button class="btn" id="retryCustomerAccount">Try again</button>`; document.getElementById('retryCustomerAccount').onclick = pageCustomerAccount; }
}

async function pageCustomerBook(privateProviderId = '') {
  const customerUser = await customerAuth.getSupabaseUser().catch(() => null);
  const [customerProfileRow, customerVehicles] = customerUser ? await Promise.all([customerAuth.getCustomerProfile(customerUser).catch(() => null), customerAuth.getCustomerVehicles(customerUser).catch(() => [])]) : [null, []];
  const customerProfile = normalizeCustomerProfile(customerProfileRow, customerUser);
  const defaultVehicle = customerVehicles.find(vehicle => vehicle.is_default) || customerVehicles[0] || null;
  app.innerHTML = `<div class="app-shell customer-book-shell"><div class="topbar customer-book-topbar"><div class="brand"><div class="drop"></div>Docket</div><div class="customer-topbar-actions"><a class="topbar-link" href="${customerUser ? '#/account' : '#/account/login'}">${customerUser ? 'My account' : 'Sign in'}</a><a class="topbar-link" href="#/">Back to home</a></div></div><div class="screen customer-book-screen"><div class="customer-book-heading"><div><div class="customer-kicker">Car wash booking</div><h1>${privateProviderId ? 'Book directly with this provider.' : 'Find a wash that fits your day.'}</h1><p class="lead">Choose a time or provider, then pick an available service.</p></div></div>${privateProviderId ? '<div id="privateProviderNotice" class="billing-notice"></div>' : ''}<div id="customerBook" class="card">Loading providers…</div></div></div>`;
  const root = document.getElementById('customerBook');
  try {
    const catalogue = await loadCatalogue({ privateProviderId });
    const providers = catalogue.providers || [];
    const locations = catalogue.locations || [];
    const services = catalogue.services || [];
    if (privateProviderId) { const provider = providers[0]; document.getElementById('privateProviderNotice').innerHTML = provider?.marketplaceVerified ? `<strong>${h(provider.name)}</strong><span>Verified marketplace provider</span>` : `<strong>${h(provider?.name || 'Direct provider booking')}</strong><span>Not yet marketplace verified. This private link books directly with the provider.</span>`; }
    ...
    const openBookingSheet = ({ providerId, locationId, serviceId, date, time }) => {
      ...
      const overlay = document.createElement('div'); overlay.className = 'modal-overlay booking-sheet-overlay'; overlay.dataset.transientOverlay = '';
      overlay.innerHTML = `<div class="modal-card booking-sheet" role="dialog" aria-modal="true" aria-labelledby="booking-sheet-title"><button class="modal-close" id="closeBookingSheet" type="button" aria-label="Close booking form">×</button><div class="eyebrow">Ready to book</div><h3 id="booking-sheet-title">${h(service?.name || 'Car wash')}</h3><div class="booking-summary"><strong>${h(provider?.name || '')}</strong><span>${h(location?.name || '')}</span><span>${h(date)} · ${h(time)}</span><span>RM ${Number(service?.price_myr || 0).toFixed(2)} · ${Number(service?.duration_minutes || 0)} min</span></div><form id="bookingSheetForm"><div class="field"><label for="sheetName">Your name</label><input id="sheetName" value="${h(customerProfile.name)}" autocomplete="name" required></div><div class="field"><label for="sheetPhone">Malaysian mobile number</label><input id="sheetPhone" value="${h(customerProfile.phone)}" type="tel" placeholder="012-3456789" autocomplete="tel" required></div>${customerVehicles.length ? `<div class="field"><label for="savedVehicleChoice">Saved vehicle</label><select id="savedVehicleChoice"><option value="">Enter another vehicle</option>${customerVehicles.map(vehicle => `<option value="${h(vehicle.id)}" ${vehicle.id === defaultVehicle?.id ? 'selected' : ''}>${h(vehicle.plate)} · ${h(vehicle.make_model)}</option>`).join('')}</select></div>` : ''}<div class="field"><label for="sheetPlate">Car plate number</label><input id="sheetPlate" value="${h(defaultVehicle?.plate || '')}" placeholder="ABC 1234" autocomplete="off" required></div><div class="field"><label for="sheetVehicle">Car make and model</label><input id="sheetVehicle" value="${h(defaultVehicle?.make_model || '')}" placeholder="e.g. Perodua Myvi" autocomplete="off" required></div><p id="sheetMessage" class="lead" role="status"></p><button class="btn" type="submit">Confirm this booking</button></form></div>`;
      document.body.appendChild(overlay); const form = overlay.querySelector('#bookingSheetForm'); const message = overlay.querySelector('#sheetMessage'); const savedChoice = overlay.querySelector('#savedVehicleChoice'); if (savedChoice) savedChoice.onchange = () => { const vehicle = customerVehicles.find(item => item.id === savedChoice.value); form.querySelector('#sheetPlate').value = vehicle?.plate || ''; form.querySelector('#sheetVehicle').value = vehicle?.make_model || ''; }; const close = () => overlay.remove(); overlay.querySelector('#closeBookingSheet').onclick = close; overlay.onclick = event => { if (event.target === overlay) close(); }; form.onsubmit = async event => { event.preventDefault(); const submit = form.querySelector('button[type="submit"]'); submit.disabled = true; message.textContent = 'Checking the slot…'; try { const result = await createBooking({ provider_id: providerId, location_id: locationId, service_id: serviceId, date, time, name: form.querySelector('#sheetName').value.trim(), phone: form.querySelector('#sheetPhone').value.trim(), vehicle_plate: form.querySelector('#sheetPlate').value.trim(), vehicle_make_model: form.querySelector('#sheetVehicle').value.trim(), access: privateProviderId ? 'private' : 'marketplace' }); overlay.querySelector('.booking-sheet').innerHTML = `<div class="eyebrow">Booking confirmed</div><h3>Your wash is booked</h3><p class="lead">${customerUser ? 'This booking is now in your Docket account.' : 'Keep this reference for your appointment.'}</p><div class="booking-summary"><strong>${h(result.reference)}</strong><span>${h(service?.name || '')}</span><span>${h(location?.name || '')}</span><span>${h(date)} · ${h(time)}</span><span>${h(form.querySelector('#sheetPlate')?.value || '')} · ${h(form.querySelector('#sheetVehicle')?.value || '')}</span><span>RM ${Number(service?.price_myr || result.service?.price_myr || 0).toFixed(2)}</span></div><div class="confirmation-actions">${customerUser ? '<a class="btn" href="#/account">View my bookings</a>' : '<button class="btn" id="bookAnother" type="button">Book another wash</button>'}<button class="btn secondary" id="closeConfirmation" type="button">Done</button></div>`; overlay.querySelector('#closeConfirmation').onclick = close; overlay.querySelector('#bookAnother')?.addEventListener('click', () => { close(); pageCustomerBook(privateProviderId); }); } catch (error) { submit.disabled = false; message.textContent = error?.message || 'Could not book this slot.'; } };
    };
...
const routes = {
  ...
  '#/staff/verification': pageProviderVerification,

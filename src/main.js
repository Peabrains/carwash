...
async function pageCustomerBook() {
  ...
  app.innerHTML = `<div class="app-shell customer-book-shell"><div class="topbar customer-book-topbar"><div class="brand"><div class="drop"></div>Docket</div><div class="customer-topbar-actions"><a class="topbar-link" href="${customerUser ? '#/account' : '#/account/login'}">${customerUser ? 'My account' : 'Sign in'}</a><a class="topbar-link" href="#/">Back to home</a></div></div><div class="screen customer-book-screen"><div class="customer-book-heading"><div><div class="customer-kicker">Car wash booking</div><h1>Find a wash that fits your day.</h1><p class="lead">Choose a time or provider, then pick an available service.</p></div></div><div id="customerBook" class="card">Loading providers…</div></div></div>`;
  ...
  try {
    ...
    const openBookingSheet = ({ providerId, locationId, serviceId, date, time }) => {
      ...
      document.body.appendChild(overlay); const form = overlay.querySelector('#bookingSheetForm'); const message = overlay.querySelector('#sheetMessage'); const savedChoice = overlay.querySelector('#savedVehicleChoice'); if (savedChoice) savedChoice.onchange = () => { const vehicle = customerVehicles.find(item => item.id === savedChoice.value); form.querySelector('#sheetPlate').value = vehicle?.plate || ''; form.querySelector('#sheetVehicle').value = vehicle?.make_model || ''; }; const close = () => overlay.remove(); overlay.querySelector('#closeBookingSheet').onclick = close; overlay.onclick = event => { if (event.target === overlay) close(); }; form.onsubmit = async event => { event.preventDefault(); const submit = form.querySelector('button[type="submit"]'); submit.disabled = true; message.textContent = 'Checking the slot…'; try { const result = await createBooking({ provider_id: providerId, location_id: locationId, service_id: serviceId, date, time, name: form.querySelector('#sheetName').value.trim(), phone: form.querySelector('#sheetPhone').value.trim(), vehicle_plate: form.querySelector('#sheetPlate').value.trim(), vehicle_make_model: form.querySelector('#sheetVehicle').value.trim() }); overlay.querySelector('.booking-sheet').innerHTML = `<div class="eyebrow">Booking confirmed</div><h3>Your wash is booked</h3><p class="lead">${customerUser ? 'This booking is now in your Docket account.' : 'Keep this reference for your appointment.'}</p><div class="booking-summary"><strong>${h(result.reference)}</strong><span>${h(service?.name || '')}</span><span>${h(location?.name || '')}</span><span>${h(date)} · ${h(time)}</span><span>${h(form.querySelector('#sheetPlate')?.value || '')} · ${h(form.querySelector('#sheetVehicle')?.value || '')}</span><span>RM ${Number(service?.price_myr || result.service?.price_myr || 0).toFixed(2)}</span></div><div class="confirmation-actions">${customerUser ? '<a class="btn" href="#/account">View my bookings</a>' : '<button class="btn" id="bookAnother" type="button">Book another wash</button>'}<button class="btn secondary" id="closeConfirmation" type="button">Done</button></div>`; overlay.querySelector('#closeConfirmation').onclick = close; overlay.querySelector('#bookAnother')?.addEventListener('click', () => { close(); pageCustomerBook(); }); } catch (error) { submit.disabled = false; message.textContent = error?.message || 'Could not book this slot.'; } };
    };
